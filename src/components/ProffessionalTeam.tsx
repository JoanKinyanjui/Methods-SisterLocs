...
<div className='grid justify-center float-animation '>
  ...
  <Image src={active?.image} alt='team' width={200} height={200} className=' rounded-t-md bg-gray_2 md:w-[250px] lg:w-[260px] h-[240px] md:h-[330px] lg:h-[320px] object-cover'/>
...
<div className='flex gap-4 md:gap-6 flex-wrap justify-center my-5'>
  {team.map((person)=>(
    <div className={` rounded-full  flex justify-center items-center  ${activeStatus === person.id ? 'border-[4px] border-[#FFAF00] scale-90 shadow-2xl': ''} `} key={person.id} onClick={()=>setActiveTeamMember(person,person.id)}>
        <Image src={person.image} alt='team' width={90} height={90} className='rounded-full w-[90px] h-[90px] sm:w-[100px] sm:h-[100px] md:w-[120px] md:h-[120px]'/>
    </div>
  ))}
</div>
</div>
